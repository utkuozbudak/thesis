...
def average_results(input_file, output_file):
    ...
    while True:
        ...
        if not all(key in data for key in run_keys):
            break  # Exit loop if no more experiments are found

        # Copy non-averaged data from the first run
        averaged_experiment = {key: data[run_keys[0]][key] for key in data[run_keys[0]] if key not in ['training_info', 'test_info', 'random_seed']}
        # Collecting 'random_seed' values and saving them in 'training_info'
        ...
        # Averaging numerical values in training_info and test_info
        for key in run_keys:
            for section in ['training_info', 'test_info']:
                for sub_key, sub_value in data[key][section].items():
                    ...
                    elif isinstance(sub_value, (int, float)):  # Average only numerical values
...
base_directory = 'outputs/burgers/results'
x_values = [2, 4, 8, 16, 32, 64, 128, 256]

for x in x_values:
    input_file = os.path.join(base_directory, f'burgers_results_n_[{x}].json')
    output_file = os.path.join(base_directory, f'burgers_results_n_[{x}]_avg.json')
